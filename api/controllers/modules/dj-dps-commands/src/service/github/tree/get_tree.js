...
module.exports = {
    name: "service.github.git.getTree",
    ...
    execute: function(command, state, config) {
        ...
        let tree_sha = command.settings.tree_sha
        let recursive = command.settings.recursive

        return new Promise((resolve, reject) => {

                gh.git.getTree({owner, repo, tree_sha, recursive})
                    .then( response => {
                        state.head = {
                            type: "json",
                            data: response
